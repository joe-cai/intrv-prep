...
public:
    ...
    string minWindow(string s, string t) {
        vector<int> count(numascii, 0); // accumulate chars in string t
        for (int i = 0; i < t.size(); i++) count[t[i]]++;
        vector<int> window(numascii, 0); // accumulate chars in string s

        int ans = INT_MAX;
        string sol = "";

        int start = 0;
        int pos = start; // from start to end is the current string
        ...
        while (start < s.size()) {
	        ...
	        // the character is not yet excessive
	    ...
	    // window[s[start]] > count[s[start]] means s[start] is redundant
            while (start < s.size() && (count[s[start]] == 0 || window[s[start]] > count[s[start]])) {
